...
namespace Microsoft.Datasync.Client.Serialization
{
    ...
    public class ServiceSerializer
    {
        ...
        public static DateTimeOffset? GetUpdatedAt(JObject item)
        {
            ...
            // Use case-insensitive lookup for compatibility with different serializers.
            if (!item.TryGetValue(SystemProperties.JsonUpdatedAtProperty, StringComparison.OrdinalIgnoreCase, out JToken valueToken))
            {
                return null;
            }
            ...
            if (valueToken.Type == JTokenType.String)
            {
                ...
                if (DateTimeOffset.TryParse(raw, null, DateTimeStyles.RoundtripKind, out var parsed))
                {
                    return parsed;
                }

                if (DateTimeOffset.TryParse(raw, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal, out parsed))
                {
                    return parsed;
                }

                // Fall back to JSON deserialization for non-standard forms, if possible.
                try
                {
                    return valueToken.ToObject<DateTimeOffset>();
                }
                catch
                {
                    return null;
