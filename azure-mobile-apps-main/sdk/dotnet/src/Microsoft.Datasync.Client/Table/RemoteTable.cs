...
namespace Microsoft.Datasync.Client.Table
{
    ...
    internal class RemoteTable : IRemoteTable
    {
        ...
        protected async Task<Page<JToken>> GetNextPageAsync(string query = "", string requestUri = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                Method = HttpMethod.Get,
                // UriPathAndQuery = requestUri ?? $"https://pulseargwebappmobile.azurewebsites.net/" + TableEndpoint + queryString,
            ...
            {
                JToken itemsToken = response.SelectToken(Page.JsonItemsProperty);
                JToken countToken = response.SelectToken(Page.JsonCountProperty);
                JToken nextLinkToken = response.SelectToken(Page.JsonNextLinkProperty);

                if (response.Type == JTokenType.Array)
                {
                    result.Items = response.ToObject<JArray>().ToList();
                }
